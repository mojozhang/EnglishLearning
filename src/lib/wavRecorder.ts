...
export class WAVRecorder {
    ...
    public onVADStateChange: ((rms: number, isSpeech: boolean, silenceDuration: number) => void) | null = null;
    ...
    async start(stream: MediaStream) {
        ...
        this.vadNode.onaudioprocess = (e) => {
            ...
            const currentSilenceDuration = this.silenceStartTime ? (Date.now() - this.silenceStartTime) : 0;
            ...
            if (currentSilenceDuration > this.SILENCE_DURATION) {
                if (this.onSilence) {
                    console.log(`Silence detected (${currentSilenceDuration}ms), trigger stop.`);
                    this.onSilence();
